...
| `verify_deployed` against the real host | **run, 2026-08-31**, against `https://upgradedev.github.io/claimready/` from this machine, through the `--verify-deployed` flag, which films nothing and spends nothing. All **26** camera sources fetched, `the deployed page is 21fc9f2, on every one of those files`, exit 0. Refused four ways in the same session: a commit the tree is not, which named `assets/styles.css`; no URL given; a URL whose files answer 404; and **one digit changed in `fixtures/insurers/northwind.json`**, an excess of 250 written as 251, which it named and refused. That fourth one is the reason the list grew: until this morning `CAMERA_PATHS` was `index.html`, `src` and `assets`, so the three JSON files that decide what every tool ANSWERS were outside the check while the check called itself every file the page loads |
