...
| `verify_deployed` against the real host | **run, 2026-08-31**, against `https://upgradedev.github.io/claimready/` from this machine, through the `--verify-deployed` flag, which films nothing and spends nothing. All **25** camera sources fetched, `the deployed page is 860e68a, on every one of those files`, exit 0. Refused four ways in the same session: a commit the tree is not, which named `assets/styles.css`; no URL given; a URL whose files answer 404; and **one digit changed in `fixtures/insurers/northwind.json`**, an excess of 250 written as 251, which it named and refused. That fourth one is the reason the list grew: until this morning `CAMERA_PATHS` was `index.html`, `src` and `assets`, so the three JSON files that decide what every tool ANSWERS were outside the check while the check called itself every file the page loads |
| `CAPTURE_JS` | syntax checked with `node --check` against the emitted file. Not executed |
| `capture_machine_beat`, Playwright | **run, 2026-08-31**, [run 33383184634](https://github.com/upgradedev/claimready/actions/runs/33383184634), capture only, on `707401a`. All three machine beats filmed against the deployed page in the Chrome Dev channel with WebMCP on: `01-problem` 17.36s in a browser with no agent, `02-publishes` 24.40s and `08-close` 25.36s in one with. Both agent beats printed `agent surface: document.modelContext, provided by the browser`, and beat 02 printed the nine tools it found, `record_supporting_details` among them. No console error, no page error, and the job ended `every machine beat filmed. The browser capture works on this runner.` Two earlier attempts failed first, in the camera and not in the page: a missing Playwright ffmpeg, and beat 01 being handed a browser that carried WebMCP when its whole subject is a browser that does not |
