...
| Commit under test | `1ee157d`. Later commits touched documentation and video tooling only, so the run still drove the bytes being served. Do not believe that sentence, check it: `git diff --stat 1ee157d "$(gh api repos/upgradedev/claimready/pages/builds/latest --jq .commit)" -- index.html src assets` prints nothing while it holds, and reads the built commit from the host rather than from this file. This matters more than it sounds: the previous run of record drove `4023446`, and `src/` changed substantially afterwards, so that run had stopped being evidence about the live page. A green run against bytes the host no longer serves is not evidence, which is the rule this file states two paragraphs down and now follows |
