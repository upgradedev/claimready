...
python video/build_video.py --verify-deployed \
  --url https://upgradedev.github.io/claimready/ --deployed-sha 357410e
```
...
`the deployed page is 357410e, on every one of those files` and exited 0.
...
taken on 2026-09-02, and `357410e` is what the host serves. The commit to name is always whichever
one the record row at the top of [docs/submission/video.md](docs/submission/video.md) declares, and
that row moves whenever a file the page loads changes. It was blank twice on 2026-09-02, each time
because work in the tree touched one of those 26 files, and the readiness row `FRZ` was red until
the release filled it in. The gate saying so is the gate working.
...
`main` once the release was served, so [run 33616908770](https://github.com/upgradedev/claimready/actions/runs/33616908770) and the deployment
name one runtime. The next one closes the same way, after the release is served and before the
...
which was `21fc9f2` rather than the `357410e` the host serves on 2026-09-02: the tools were
