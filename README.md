...
python video/build_video.py --verify-deployed \
  --url https://upgradedev.github.io/claimready/ --deployed-sha e942ee3
```
...
`the deployed page is e942ee3, on every one of those files` and exited 0.
...
taken on 2026-09-02, and `e942ee3` is what the host serves on that date. The commit to name is
always whichever one the record row at the top of
[docs/submission/video.md](docs/submission/video.md) declares, and that row moves whenever a file
the page loads changes. **That row is blank right now**, and it says why: filing integrity work in
the tree changes `src/core/claim.js`, one of the 26 files above, so `e942ee3` is not the commit the
video will be recorded against and the readiness row `FRZ` is red until the release fills the row
in. This is the second time the row has been blank for that reason on 2026-09-02, and the gate
saying so is the gate working.
...
above over all 26 files. A third gap opened on 2026-09-02, when the correctness work package changed files the page loads,
and it was closed the same way: the workflow was dispatched against `main` once the release was
served, so run 33600367240 and the deployment name one runtime. **A fourth is open as this is
written**, because the filing integrity work in the tree changes `src/core/claim.js`, and it closes
the same way, by dispatching the workflow against `main` after the release is served and before the
...
which was `21fc9f2` rather than the `e942ee3` the host serves on 2026-09-02: the tools were
...
| Handler packet after a human files | built | `node --test tests/unit/packet.test.js` and `tests/unit/packet_is_not_a_tool.test.js`. A filed claim produces a canonical JSON packet and a readable view carrying the facts, the clause and excess, every intake requirement with what answered it, the pinned rows, the route each answer took and the tool calls, digested with SHA-256. `node scripts/verify_packet.mjs <file>` recomputes it and exits 1 when a character moved, which was demonstrated by changing one severity. That script imports the same module that produced the digest, so it shows the content in front of it matches the digest beside it and shows nothing about the algorithm: [docs/handler-verification.md](docs/handler-verification.md) gives two routes that use none of our code and shows all three agreeing on one worked example. Say what a bare SHA-256 is worth and no more. There is no key and no signature here, so a matching digest catches a packet that changed in transit and two copies that have drifted apart, and it does not show page origin, authorship, that nobody edited the content and recomputed the digest to match, immutable storage, or that any insurer received it. The verifier checks the SHAPE before the digest, against the same schema `src/core/packet.js` runs before it seals anything, and exits 2 when a key this build writes is missing or holds the wrong shape. It does NOT refuse a document that carries EXTRA keys: a forger can add an insurer_receipt beside the ones the schema knows and the verifier walks past it, inside the digested region or outside it. So it answers whether the packet this build made is intact, not whether somebody added something to it: `tests/unit/packet_schema.test.js` walks 40 single value breaks, each of which used to verify with a correct digest. It refuses for a draft that was never filed, for no pack, for a half built pack, for another insurer's rules, for a filed status the gate could not have granted, and for a filed status written by hand: `fileClaim` records the claim objects it actually filed in a set held privately in `src/core/claim.js`, and the packet will not describe a filing this page did not perform. That record lives in one browser tab and dies with it, so it stops this page overclaiming and proves nothing to a reader holding the file. `src/webmcp` never imports the module, so no registered tool builds it or hands it back |
...
| The tool surface running in a real browser's own WebMCP implementation | proven in CI on 2026-09-02, against the bytes the host served that day, and **not yet against the runtime this entry will record** | [run 33600367240](https://github.com/upgradedev/claimready/actions/runs/33600367240), workflow `WebMCP evals`, conclusion success, `headSha` `12f7935`, which is a documentation only commit over a runtime of `e942ee3`. Two jobs. The smoke evals reported `Passed steps: 16/16 across 3 case(s)` against the deployed page, driven by Chrome's own `webmcp-evals` harness, which launches Chrome with `--enable-features=WebMCP`. No shim of ours is involved. The negative control in the same run reported `Passed steps: 7/8 across 1 case(s)` with the verdict `PROVEN`, because its eighth step is REQUIRED to fail: the ninth tool has to be gone after a patch that puts the car back on the road. The second job is our own dependency free `node evals/browser_probe.mjs`, which printed `probe: PASS. 81 checks against the deployed page, none failed`. **81 is the current number and 71 is not.** On 2026-09-02 the note phase and the declarative phase were each found to pass a forged transcript, closing them took the matrix from 71 to 81, and the transcript shape changed with it, so the probe as it is now prints 81 and cannot reproduce the 71 that [run 33560224732](https://github.com/upgradedev/claimready/actions/runs/33560224732) printed at `c93b138` on 2026-09-01. Ten checks were added to the oracle, not to the product. **What this row does not say.** Filing integrity work sitting in the working tree changes `src/core/claim.js`, one of those 26 files, so no run named here describes what a judge opens once that is released, and the workflow has to be dispatched against `main` again afterwards. **Why this row is worded so carefully.** It read `against the bytes the host serves` twice before while naming a commit five changes behind what was deployed, and both times it was wrong. So do not take its word for what is served: `python video/build_video.py --verify-deployed --url https://upgradedev.github.io/claimready/ --deployed-sha e942ee3` fetches all 26 files the page loads and refuses if one of them differs. Run on 2026-09-02 from a clone at `e942ee3` it printed `the deployed page is e942ee3, on every one of those files` and exited 0. |
