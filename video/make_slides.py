...
and the whole of the close have to fit in what is left, and they are written to 12 and 6 seconds
rather than to what would be comfortable. Raising the cap was the other way to make room and it was
...
def context_beat():
    """
    Twelve seconds to answer the question the recording never answers: what is this about.

    Three cards. What happened, what the industry calls the next thing that happens, and why
    getting it wrong costs a second phone call. The third is the product's whole reason to exist,
    so it gets the longest hold and the only rule.
    """
    return [
        text(1, "A van reverses into a parked car.", font=FONT_BOLD, size=74, colour=INK,
             y=430, start=0.15, stop=4.10),
        text(2, "Nobody is hurt. The paperwork starts.", font=FONT, size=44, colour=INK_SOFT,
             y=545, start=0.95, stop=4.10),

        text(3, "FIRST NOTICE OF LOSS", font=FONT_BOLD, size=64, colour=ACCENT,
             y=420, start=4.25, stop=8.10),
        rule(660, 1260, 505, 3, ACCENT, 4.55, 8.10),
        text(4, "The first form an insurer sees.", font=FONT, size=46, colour=INK,
             y=560, start=4.95, stop=8.10),
        text(5, "Every motor claim in the world starts on one.", font=FONT, size=46,
             colour=INK_SOFT, y=630, start=5.45, stop=8.10),

        text(6, "One field the policy needed and nobody asked for", font=FONT_BOLD, size=58,
             colour=INK, y=420, start=8.25, stop=11.95),
        text(7, "is a second call to the driver, days later.", font=FONT, size=52,
             colour=YOU, y=520, start=8.95, stop=11.95),
        rule(560, 1360, 615, 3, YOU, 9.25, 11.95),
        text(8, "The rules that decide it live on the insurer's page.", font=FONT, size=42,
             colour=INK_SOFT, y=680, start=9.95, stop=11.95),
    ]
...
def main():
    ...
    render("00-context", 12.0, context_beat())
    render("09-closing", 6.0, closing_beat())
