...
| Length of the finished cut | **169.30s**, under the 170s pipeline cap and under the organizer's three minute rule. Built by [run 33780012606](https://github.com/upgradedev/claimready/actions/runs/33780012606), whose sync gate printed `G  total under cap    169.300s of 170s, 0.700s spare` and passed all eight rows. **This cell quoted that line as `169.400s` until 2026-09-04, a number the gate did not print**, beside a cell that gives the length correctly as 169.30s. The published video reports `"lengthSeconds":"169"`. This cell read **not measured** until the takes were recorded on 2026-09-03 |
...
| Corrections to the narration | **Two, and both are published beside the video rather than re-cut.** **At 1:40** the narration says the roadside collection "is arranged". The demo records a local roadside assistance request and marks that requirement answered: `src/ui/app.js:1393` sets `ui.assistanceAt` and nothing else happens, and the only `fetch` calls in `src/` are the two same origin fixture loads at boot, `app.js:362` and `app.js:1479`. It does not arrange or dispatch a service and nothing leaves the page. **At 2:08** it says "Chrome builds the schema from that markup", where the accurate word is the browser: that take was shot in the ChatGPT desktop app's built in browser, and the declarative half is verified separately in Chrome `151.0.7922.174` stable. Both timings are read off the cut's own `captions.vtt`, cues 31 and 39, from [run 33780012606](https://github.com/upgradedev/claimready/actions/runs/33780012606), the run that built the published cut. Both are written into the YouTube description under `TWO CORRECTIONS TO THE NARRATION` and into the Devpost entry, on 2026-09-04. **Neither is a reason to unfreeze**: the fix is one word in each and a re-upload costs the video URL, which the Devpost field, the README and this file all name |
| The three commits, in one chain | **A reader asked why the takes, the build and the live page name different commits, and the answer is that they are three different events.** **`61b4d8b`** is what the six owner takes were shot against: it was the declared freeze on 2026-09-03 and the page they filmed. **`747b082`** is the checkout the cut was built from, in [run 33780012606](https://github.com/upgradedev/claimready/actions/runs/33780012606), which is what `manifest.json` records as `deployed_sha`. It carries the same on camera bytes as `61b4d8b`: `git diff --stat 61b4d8b 747b082 -- index.html src assets fixtures` is empty, and both print the same `camera_digest`, `e616607b578ac401`. It is a later commit only because the takes and the slide beats were committed on it, and it reached `main` squashed as `5225667`. **`ecd4c09`** is what the host serves now and the current freeze, declared 2026-09-04. The only on camera file that moved between `61b4d8b` and it is `index.html`, and the diff is two blocks of markup no beat puts on camera: a link inside a collapsed `details` element, and a panel subtitle. `git diff 61b4d8b ecd4c09 -- index.html src assets fixtures --stat` names that one file. **So no frame in the published cut differs from what a visitor gets**, and the chain is takes `61b4d8b`, build `747b082`, runtime `ecd4c09` |
...
### Seven superseded declarations, each named with its reason
...
**`61b4d8b`, declared 2026-09-03 and superseded on 2026-09-04.** Two things a visitor reads on the
page, and neither was true. The help disclosure offered `the short setup note in the README` and
pointed at `./README.md` on this origin; that file is 87,209 bytes and GitHub Pages serves it as
`text/markdown`, so the one link offered to somebody who could not find an agent returned raw markup
rather than a page. And the claim draft said every row shows whether an answer arrived through a
control on this page or through a tool call, where the page shows three routes: `BADGE_WORDS` at
`src/ui/render.js:80` maps `policy` to `on file`, and `src/core/claim.js:630` writes `policy` on
every field the synthetic incident arrives with, so `on file` is on screen before a visitor touches
anything. **This is the first unfreeze that costs neither a re-cut nor a re-shoot.** Both edits are
inside markup no beat opens: the help block is a collapsed `details` element and no `beat.json`
touches it, and the draft blurb is a panel subtitle. The takes shot against `61b4d8b` are unchanged
and still describe the served runtime, and the only cost was a deploy and a fresh browser run.
