...
| | |
| --- | --- |
| **Try it.** No account, nothing to install | <https://upgradedev.github.io/claimready/> |
...
tutorial, and it is long on purpose: every number carries the command that produced it, and every
correction is left visible with the date it was made instead of being tidied away. If you read three
sections, read [Why this needs WebMCP](#why-this-needs-webmcp), [What is a tool here, and what is
deliberately not](#what-is-a-tool-here-and-what-is-deliberately-not) and [Status](#status). The two
notes below are kept for the same reason and folded away so they are not the first thing you meet.

<details>
<summary><strong>What the three badges mean, and why one of them is allowed to be red</strong></summary>
...
deliverable rows, and it is red on purpose while any mandatory deliverable is missing. The public
video is one of those rows. **It was missing when this paragraph was written and it is not now**: it
is at <https://youtu.be/cazdzwy2qKU>, and `node scripts/readiness.mjs` prints `D4    PASS` and
`deliverable rows outstanding: 0`. The readiness gate
...
<details>
<summary><strong>One note on the name, before you go looking for it</strong></summary>

ClaimReady is not a unique name. Search it
...
Chrome 151.0.7922.174, stable channel, against the page as deployed at `21fc9f2`**, which is not what
the host serves now: the tools were
published, executed, and one of them was withdrawn when the claim changed under it. **This sentence
also dated that session 2026-08-31 until 2026-09-04**, and `git log -1 --format=%cI 21fc9f2` prints
`2026-09-01T04:26:27+03:00`, so the two cannot both be right. The commit is kept because it can be
checked and the date is dropped because it cannot, which is the same call the table above records. Reproduce with
...
`Filing the claim is a control on this page and is not exposed as a WebMCP tool.`, from
`src/webmcp/tools/read_claim_state.js:155`, and `Filing is a control on this page and is not
exposed as a WebMCP tool.` from `src/webmcp/tools/validate_claim.js:44`.

**This line quoted a sentence the page has never said, until 2026-09-03.** It read `Filing the
claim is a button pressed by the person on the page. It is not available as a tool.`, which
`grep -rF` finds nowhere under `src/`. It is the second invented quotation this repository has
shipped, and it invented the same thing both times: the real wording names a CONTROL, a surface,
and the invention named a PERSON pressing a button, which is the authorship claim this page is
careful never to make. Quote the code or do not use quotation marks.
...
`MANDATORY, what the rules require:   5 of 5 PASS  (LIVE PASS, LIC PASS, D1 PASS, D3 PASS, D4 PASS)`,
a `READY TO SUBMIT` tally, `deliverable rows outstanding: 0`, and
`READY. The owner gated rows above are still owed by a person.` **It exits 0.**

This step told a judge to expect `NOT READY. A mandatory deliverable is missing: D4.` and an exit
of 1 until 2026-09-03, which was true while the video did not exist. The video was published that
day, `D4` went green, and the step was left describing the old answer. A judge who pastes the
command in a section whose whole point is that every number arrives with the command that makes
it, and gets the opposite of what they were told, has no way to know which of the two is wrong.
...
`MANDATORY, what the rules require:   5 of 5 PASS  (LIVE PASS, LIC PASS, D1 PASS, D3 PASS, D4 PASS)`,
`automated rows:   27 of 27 PASS, 100 percent`, `READY TO SUBMIT:  27 of 31 proven, 87.1 percent`
and `deliverable rows outstanding: 0`, exit 0. **This paragraph gave the 4 of 5 and 26 of 27 that
the gate printed while the video did not exist**, and was left standing after it did. The four rows
still outstanding are the owner gated ones, which no script can prove. The `LIVE` row is the one to read first: it fetches
...
| Public video | **published 2026-09-03, <https://youtu.be/cazdzwy2qKU>, 2:49 against the organizer's three minute cap** | The gate reads the link out of [docs/submission/video.md](docs/submission/video.md), and `node scripts/readiness.mjs` prints `D4    PASS` and `deliverable rows outstanding: 0`, exit 0. Checked from outside any account with `curl -s -L "https://www.youtube.com/watch?v=cazdzwy2qKU"`, which returns `"isPrivate":false`, `"isUnlisted":false` and `"lengthSeconds":"169"`. **This row read `not yet built` until 2026-09-03**, which was true while it was true and was left standing for some hours after it stopped being, one row above the command that refutes it |
| Written description | **pasted into the Devpost submission, 2026-09-03, and the entry reads Submitted** | [docs/submission/description.md](docs/submission/description.md) is the source, and `node scripts/readiness.mjs` row `D3` reads `PASS`. The published page carries two sections this file does not mirror, Evidence and What the demo video shows, which were written for the form. This row read `drafted, not yet pasted` until 2026-09-03 |
